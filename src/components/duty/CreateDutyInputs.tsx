...
export const CreateDutyInputs: FC<DutyInputsProps> = (props) => {
  ...
  const onsubmit = async (data: any) => {
    ...
    const moreDuties = await fetch(
      `/api/internal/more-resources?resource=duty&projectId=${project?._id}`
    )
    const canCreateMore = await moreDuties.json()

    if (!canCreateMore) {
      toast.error('You have reached your limit of duties')
      return
    }
